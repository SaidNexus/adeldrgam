...
async def get_articles_count(session: Session = Depends(get_session)):
    """Get total number of articles safely."""
    try:
        count = session.scalar(select(func.count(Article.id))) or 0
        return {"total_articles": int(count)}
    except Exception as e:
        print(f"Error fetching articles count: {e}")
