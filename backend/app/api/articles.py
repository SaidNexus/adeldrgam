...
def get_articles(
    ...
    try:
        ...
        if author_id:
            ...
            # Direct check if User is already joined by checking the from clause
            from sqlalchemy.orm.util import _is_mapped_class
            has_user = any(
                hasattr(f, 'entity_namespace') and f.entity_namespace is User 
                for f in query.get_final_froms()
            ) if hasattr(query, 'get_final_froms') else False
