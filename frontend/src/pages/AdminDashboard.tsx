...
export const AdminDashboard = () => {
    ...
    return (
        <div className="bg-gray-50 min-h-screen flex selection:bg-blue-100 selection:text-blue-900 font-almarai relative overflow-x-hidden">
            ...
            {/* Admin Sidebar */}
            <aside className={`
                w-80 bg-white border-e border-gray-100 fixed lg:sticky top-0 h-screen flex flex-col p-8 z-[70] transition-transform duration-300 lg:translate-x-0
            ...
            `}>
                ...
                <nav className="flex-1 space-y-2">
                    <NavButton active={activeTab === 'overview'} onClick={() => { setActiveTab('overview'); setIsSidebarOpen(false); }} icon={<LayoutDashboard />} label="نظرة عامة" />
                    <NavButton active={activeTab === 'content'} onClick={() => { setActiveTab('content'); setIsSidebarOpen(false); }} icon={<FileText />} label="إدارة المحتوى" />
                    <NavButton active={activeTab === 'users'} onClick={() => { setActiveTab('users'); setIsSidebarOpen(false); }} icon={<Users />} label="إدارة المستخدمين" />
                    <NavButton active={activeTab === 'categories'} onClick={() => { setActiveTab('categories'); setIsSidebarOpen(false); }} icon={<Layers />} label="التصنيفات" />
                    <NavButton active={activeTab === 'publishers'} onClick={() => { setActiveTab('publishers'); setIsSidebarOpen(false); }} icon={<UserCheck />} label="طلبات الانضمام" />
                </nav>
            ...
            {/* Main Content Area */}
            <main className="flex-1 min-h-screen px-4 md:px-12 py-8 lg:py-16 overflow-x-hidden">
                <div className="max-w-7xl mx-auto space-y-8 md:space-y-12">
                    ...
                    <div className="lg:hidden flex items-center justify-between mb-8">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-lg font-black text-gray-900 uppercase">Admin</span>
                        </div>
                        <button
                            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                            className="h-12 w-12 flex items-center justify-center bg-white border border-gray-100 rounded-xl text-gray-600 shadow-sm"
                        >
                    ...
                    <div className="lg:hidden flex overflow-x-auto gap-3 pb-6 -mx-4 px-4 no-scrollbar scroll-smooth">
                        <MobileNavButton active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} icon={<LayoutDashboard size={18} />} label="نظرة عامة" />
                        <MobileNavButton active={activeTab === 'content'} onClick={() => setActiveTab('content')} icon={<FileText size={18} />} label="المحتوى" />
                        <MobileNavButton active={activeTab === 'users'} onClick={() => setActiveTab('users')} icon={<Users size={18} />} label="المستخدمين" />
                        <MobileNavButton active={activeTab === 'categories'} onClick={() => setActiveTab('categories')} icon={<Layers size={18} />} label="التصنيفات" />
                        <MobileNavButton active={activeTab === 'publishers'} onClick={() => setActiveTab('publishers')} icon={<UserCheck size={18} />} label="الطلبات" />
                    </div>
...
const MobileNavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: any, label: string }) => (
    <button
        onClick={onClick}
        className={`flex items-center gap-2 px-5 py-3 rounded-xl font-bold whitespace-nowrap transition-all border ${active ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-500/20' : 'bg-white text-gray-500 border-gray-100 hover:bg-gray-50'
            }`}
    >
        {icon}
        {label}
        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {Array(4).fill(0).map((_, i) => (
                    <div key={i} className="h-48 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm animate-pulse" />
                ))}
            </div>
        );
    }
    ...
    if (isError) {
        return (
            <div className="bg-red-50 border border-red-100 p-8 rounded-[2.5rem] text-center space-y-4">
                ...
                <h3 className="text-xl font-black text-red-900">فشل تحميل البيانات</h3>
                <p className="text-red-600 font-medium">يرجى المحاولة مرة أخرى لاحقاً أو التحقق من اتصالك بالقاعدة.</p>
            </div>
        );
    }
...
const StatsCard = ({ label, value, icon, index, isAlert }: { label: string, value: number, icon: any, index: number, isAlert?: boolean }) => (
    <motion.div
        ...
        className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6 group hover:shadow-xl hover:shadow-gray-200/50 transition-all"
    >
                                ...
                                <td className="px-10 py-6">
                                    <div className="flex items-center justify-center gap-2">
                                        {art.status !== 'published' && (
                                            <button onClick={() => approve(art.id)} className="h-11 w-11 flex items-center justify-center bg-gray-50 hover:bg-green-50 text-green-600 rounded-xl transition-all border border-gray-100" title="قبول">
                                                <CheckCircle2 className="h-5 w-5" />
                                            </button>
                                        )}
                                        {art.status === 'published' && (
                                            <button onClick={() => reject(art.id)} className="h-11 w-11 flex items-center justify-center bg-gray-50 hover:bg-orange-50 text-orange-600 rounded-xl transition-all border border-gray-100" title="رفض">
                                                <XCircle className="h-5 w-5" />
                                            </button>
                                        )}
                                        <Link to={`/articles/${art.slug}`} className="h-11 w-11 flex items-center justify-center bg-gray-50 hover:bg-blue-50 text-blue-600 rounded-xl transition-all border border-gray-100" title="عرض">
...
const UsersPanel = ({ openConfirm, userSearchQuery, setUserSearchQuery }: { openConfirm: any, userSearchQuery: string, setUserSearchQuery: any }) => {
    const queryClient = useQueryClient();
    const [debouncedSearch, setDebouncedSearch] = useState(userSearchQuery);

    useEffect(() => {
        const timer = setTimeout(() => setDebouncedSearch(userSearchQuery), 500);
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="flex items-center justify-between px-2">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">إجمالي المسجلين: {total}</span>
            </div>
            {/* User Search */}
                    ...
                    <table className="w-full text-right min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={4} className="px-10 py-16 text-center text-red-500 font-bold">
                                        حدث خطأ أثناء تحميل المستخدمين. يرجى المحاولة مرة أخرى.
                                    </td>
                                </tr>
                            ) : (!users || users.length === 0) ? (
                                <tr>
                                    <td colSpan={4} className="px-10 py-16 text-center text-gray-400 font-bold italic">
                                        {debouncedSearch ? `لا يوجد مستخدمون يطابقون "${debouncedSearch}"` : 'لا يوجد مستخدمون حالياً.'}
                                    </td>
                                </tr>
                            ) : users.map((user) => (
                                    ...
                                    <td className="px-10 py-6">
                                        <div className="flex items-center justify-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className="h-11 w-11 flex items-center justify-center bg-gray-50 text-red-400 rounded-xl border border-gray-100 hover:bg-red-50 hover:text-red-600 transition-all shadow-sm"
                                                title="حذف المستخدم"
                                            >
