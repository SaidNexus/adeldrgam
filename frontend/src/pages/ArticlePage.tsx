...
export const ArticlePage = () => {
    ...
    const likeMutation = useMutation({
        ...
        onMutate: async () => {
            ...
            const previousArticle = queryClient.getQueryData<Article>(['article', slug]);
            if (previousArticle) {
        ...
        <div className="bg-gray-50 min-h-screen flex items-center justify-center p-6">
            <div className="text-center space-y-8 max-w-md">
                ...
                <div className="space-y-3">
                    <h1 className="text-gray-900 text-3xl font-black tracking-tight">نبض غير متصل</h1>
        ...
        <div className="bg-white min-h-screen selection:bg-blue-600 selection:text-white pb-32">
                    ...
                    >
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                            <span className="px-5 py-2 bg-blue-600 text-white rounded-full text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-blue-500/20">
                                {article.category?.name_ar || 'مقال نبض'}
                            </span>
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-5 pt-4">
                            <Link to={`/profile/${article.author?.id}`} className="h-16 w-16 rounded-2xl overflow-hidden border-2 border-white shadow-xl bg-gray-100 shrink-0">
                            ...
                            <div className="flex flex-col text-right">
                                <Link to={`/profile/${article.author?.id}`} className="text-xl font-black text-gray-900 hover:text-blue-600 transition-colors">
                                    {article.author?.full_name || article.author?.username}
                                </Link>
                                <div className="flex items-center gap-3 text-sm font-bold text-gray-400">
                                    <Calendar className="h-4 w-4" />
                                    <span>نُشر في {formatDate(article.created_at)}</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
            ...
            <div className="max-w-7xl mx-auto px-6 pt-24 relative grid lg:grid-cols-[1fr_minmax(auto,800px)_1fr] gap-12">
                ...
                <aside className="hidden lg:block sticky top-32 h-fit space-y-6">
                    <div className="flex flex-col gap-4 p-4 bg-gray-50/50 border border-gray-100 rounded-3xl backdrop-blur-sm">
                        <button
                            ...
                            className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all shadow-sm active:scale-90 ${article.liked_by_me
                                ? 'bg-rose-500 text-white shadow-rose-500/20'
                                : 'bg-white text-[#64748b] hover:text-rose-500 hover:border-rose-100 border border-[#e2e8f0]'}`}
                        >
                            <Heart className={`h-6 w-6 ${article.liked_by_me ? 'fill-current' : ''}`} />
                        </button>
                        <div className="text-center text-[10px] font-black text-gray-400 uppercase tracking-widest -mt-2">
                            {nf.format(article.likes_count)}
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1 text-gray-300">
                            ...
                            <span className="text-[10px] font-black">{nf.format(article.views_count)}</span>
                        </div>
                    </div>
                </aside>
                ...
                <main className="space-y-16 lg:col-start-2">
                    <div
                        className="prose prose-2xl prose-gray max-w-none text-[#1e293b] selection:bg-blue-100 leading-[1.8] font-['Almarai']"
                        style={{ fontSize: '1.25rem' }}
                        dangerouslySetInnerHTML={{
                            __html: typeof article.content === 'string' ? article.content : article.content?.html || '',
                        }}
                    />
                    ...
                    <div className="pt-16 border-t border-gray-100 flex flex-wrap items-center justify-between gap-8">
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className={`flex items-center gap-3 h-14 px-8 rounded-2xl font-black transition-all ${article.liked_by_me
                                    ? 'bg-rose-500 text-white shadow-xl shadow-rose-500/20'
                                    : 'bg-gray-50 text-gray-400 hover:bg-rose-50 hover:text-rose-500 border border-transparent'}`}
                            >
                                <Heart className={`h-5 w-5 ${article.liked_by_me ? 'fill-current' : ''}`} />
                                {nf.format(article.likes_count)} إعجاب
                            </button>
                            <button
                                onClick={handleShare}
                    ...
                    <section className="bg-[#0f172a] rounded-[3rem] p-10 md:p-14 text-white relative overflow-hidden group border border-[#334155]">
                        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-10">
                            <div className="h-32 w-32 rounded-[2.5rem] overflow-hidden border-4 border-white/10 shrink-0 bg-gray-800 shadow-2xl">
                                {article.author?.avatar_url ? (
                                    <img src={article.author.avatar_url} alt={article.author.username} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                ) : (
                            ...
                            <div className="space-y-6 text-center md:text-right flex-1">
                                <div className="space-y-1">
                                    <h3 className="text-3xl font-black tracking-tight">{article.author?.full_name || article.author?.username}</h3>
                                    <p className="text-blue-400 font-bold uppercase text-[10px] tracking-widest">خبير ومساهم في نبض</p>
                                </div>
                                <p className="text-gray-400 text-lg leading-relaxed italic max-w-xl">
                                    {article.author?.bio || "هذا الكاتب يشاركنا أفكاره ورؤاه العميقة عبر منصة نبض لإثراء المحتوى الفصيح والمؤثر."}
                                </p>
                    ...
                    <div className="space-y-10 pt-16">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-4xl font-black text-[#0f172a] tracking-tight">حوار القراء</h2>
                        </div>
                        <div className="bg-[#f8fafc] rounded-[3rem] p-4 border border-[#e2e8f0] shadow-sm transition-colors">
            ...
            <div className="lg:hidden fixed bottom-6 left-6 right-6 z-[60] flex items-center justify-between p-2 bg-gray-900/90 backdrop-blur-xl rounded-[2rem] border border-white/10 shadow-2xl">
                <div className="flex items-center gap-2 px-4">
                    <Heart className={`h-5 w-5 ${article.liked_by_me ? 'text-rose-500 fill-current' : 'text-gray-400'}`} />
                    <span className="text-white font-black text-sm">{nf.format(article.likes_count)}</span>
                </div>
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`h-12 px-6 rounded-[1.25rem] font-black text-sm transition-all ${article.liked_by_me
                            ? 'bg-rose-500 text-white'
                            : 'bg-white text-gray-900'}`}
                    >
                        {article.liked_by_me ? 'تم الإعجاب' : 'إعجاب'}
                    </button>
                </div>
            </div>
        </div>
